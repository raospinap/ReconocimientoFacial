...
# ==========================================================
def ai_camera_worker(mode, name, code, active_class_id, allowed_students, result_queue, stop_event):
    """
    Este proceso corre en un núcleo de CPU distinto al de la GUI.
    Se comunica mediante una Queue y un Event para cierre atómico.
    """
    ...
    # Carga de perfiles para validación de duplicados[cite: 3]
    ...
    while not stop_event.is_set():
        ...
        # 2. Lógica de Enrolamiento (RF-01)
        if mode == "enroll":
            if enroll_stage < 3:
                ...
                if key == ord(' ') and face_ready and not is_processing:
                    is_processing = True
                    try:
                        # Extracción del vector actual mediante ArcFace
                        ...
                        # Validación de duplicados (Umbral estricto 0.70)
                        duplicate = False
                        if enroll_stage == 0:
                            for uid, data in all_profiles.items():
                                # Verificación explícita de None para evitar error de ambigüedad en NumPy[cite: 1]
                                ...
                                if stored is not None:
                                    ...
                                    for v in search_list:
                                        ...
                                        if dist > 0.70: # Umbral de Bloqueo por Duplicado[cite: 1]
                                            duplicate = True
                                            break
                                if duplicate: break
                ...
                # Persistencia cifrada del nuevo perfil[cite: 3, 5]
                all_profiles[code] = {
                    "nombre": name,
        ...
        # 3. Lógica de Asistencia (Umbral 0.82)[cite: 1]
                    ...
                    for uid, data in all_profiles.items():
                        ...
                        if stored is not None:
                            ...
                            for v in search_list:
                                v_arr = np.array(v)
                                dist = np.dot(emb, v_arr) / (np.linalg.norm(emb) * np.linalg.norm(v_arr))
                                if dist > 0.82 and dist > best_dist: # Umbral de Identificación[cite: 1]
...
class ReconApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        # Sincronización y comunicación entre procesos[cite: 1]
        ...
        self._setup_ui()
        # Protocolo para cierre atómico de la aplicación
    ...
    def _create_sidebar(self):
        ...
        ctk.CTkButton(self.sidebar, text="Salir", fg_color="#e74c3c", hover_color="#c0392b", command=self._on_closing).pack(pady=5, padx=20)
    ...
    def _launch_enroll_worker(self):
        ...
        if n and c:
            ...
            if any(str(k) == str(c) for k in profiles.keys()):
                self.enroll_status.configure(text=f"ERROR: EL CÓDIGO {c} YA ESTÁ EN USO", text_color="#e74c3c")
                return
    ...
    def _listen_for_result(self):
        try:
            ...
            if res == "SUCCESS":
                ...
                self.ent_name.delete(0, 'end'); self.ent_code.delete(0, 'end')
            elif res == "DUPLICATE":
                self.enroll_status.configure(text="❌ ERROR: EL ROSTRO YA EXISTE EN EL SISTEMA", text_color="#e74c3c")
            elif res == "CANCELLED":
                self.enroll_status.configure(text="⚠️ PROCESO FINALIZADO POR USUARIO", text_color="#f1c40f")
        except:
    ...
    def show_live_attendance(self):
        ...
        self.worker_process = multiprocessing.Process(
            target=ai_camera_worker, 
            args=("attendance", None, None, self.current_session['clase'], allowed, self.result_queue, self.stop_event)
        )
        self.worker_process.start()
        self._listen_for_result()
    ...
    def _on_closing(self):
        """Cierre atómico para evitar procesos fantasma[cite: 1]."""
        self.stop_event.set()
