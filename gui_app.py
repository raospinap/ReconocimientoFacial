...
class ReconApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Sistema Biométrico UT v3.2")
    ...
    def _listen_for_result(self):
        try:
            ...
            if res == "SUCCESS":
                self.enroll_status.configure(text="✅ REGISTRO EXITOSO", text_color="#2ecc71")
                self.ent_name.delete(0, 'end'); self.ent_code.delete(0, 'end')
            elif res == "DUPLICATE":
                self.enroll_status.configure(text="❌ ERROR: ROSTRO YA REGISTRADO", text_color="#e74c3c")
            elif res == "CANCELLED":
                self.enroll_status.configure(text="⚠️ OPERACIÓN CANCELADA", text_color="#f1c40f")
        except:
